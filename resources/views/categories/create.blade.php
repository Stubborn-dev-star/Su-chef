...
<div class="bg-suBg min-h-screen" style="padding-top: 100px;">
    <div class="max-w-2xl mx-auto px-6 py-12">
        ...
        {{-- Header --}}
        <div class="mb-10">
            <h1 class="font-serif text-4xl font-bold text-suText mb-2">Add New Category</h1>
            <p class="text-gray-500">Create a new category to organise recipes.</p>
        </div>
        ...
        <form method="POST" action="{{ route('categories.store') }}" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm">
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Nigerian Dishes, Breakfast, Vegan"
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm">
                ...
                <select name="type" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary">
                    ...
                    <option value="meal" {{ old('type') === 'meal' ? 'selected' : '' }}>Meal Type</option>
                    <option value="cuisine" {{ old('type') === 'cuisine' ? 'selected' : '' }}>Cuisine</option>
                    <option value="diet" {{ old('type') === 'diet' ? 'selected' : '' }}>Dietary</option>
                </select>
